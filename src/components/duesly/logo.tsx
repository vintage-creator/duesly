...
export function DueslyLogo({ className, light = false }: { className?: string; light?: boolean }) {
  return (
    <Link to="/" className={cn("inline-flex items-center gap-2 font-display font-bold tracking-tight", className)}>
      <span className="relative grid h-9 w-9 place-items-center rounded-xl bg-gradient-emerald shadow-emerald">
      ...
      <span className={cn("text-xl", light ? "text-white" : "text-navy")}>Duesly</span>
    </Link>
  );
}
